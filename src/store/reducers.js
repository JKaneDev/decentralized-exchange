...
function exchange(state = {}, action) {
	...
	switch (action.type) {
		...
		case 'ORDER_FILLED':
			...
			index === -1 ? (data = [...state.filledOrders.data, action.order]) : (data = state.filledOrders.data);
			// Update filled orders in state
			return {
				...state,
				orderFilling: false,
				filledOrders: {
		...
		case 'ORDER_MADE':
			...
			index === -1 ? (data = [...state.allOrders.data, action.order]) : (data = state.allOrders.data);
			return {
				...state,
